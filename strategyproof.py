...
if __name__ == '__main__':
    ...
    parser.add_argument('--C_min', type=int, help='min cost of a product', default=1)
    parser.add_argument('--C_max', type=int, help='max cost of a product', default=2)
    parser.add_argument('--b', type=int, help='budget', default=3)
    ...
    if not args.C:
        costs = np.random.randint(args.C_min, args.C_max, size=args.p)
    else:
    ...
    result = [check_strategy_proof(profile_, cost_dict, possible_ballots_, args.n, args.b) for i in range(args.sample_size)]
