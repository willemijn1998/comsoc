...
def plot_it(prods=10, voters=[5], cost_iterations=1, C_max=2, sample_size=100, experiment='uniform'):
    ...
    plt.savefig(f"data/figure_{experiment}_prods{prods}_cost{C_max}_size{sample_size}_{now}.jpg")
    plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', type=str, default='uniform', choices=['distributions', 'uniform'])

    args = parser.parse_args()

    if args.experiment == 'distributions':
        from strategyproof import main_path as main
